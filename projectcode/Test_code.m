...
refer_Velo = unit_vector;
refer_Force = unit_vector;

unit_vector(1,:)= (position_hist(2,:) - position_hist(1,:)) / sum(position_hist(2,:) - position_hist(1,:));
...
%% Plot
% for i = 1:length(position_hist)
%     figure(4)
%     plot(result(:,1),result(:,2), 'b');
%     hold on
%     plot(position_hist(i,1), position_hist(i,2),'ro');
%     axis([-0.1,1.1,-0.1,1.1])
%     
%     F_arraw_x = [position_hist(i,1), position_hist(i,1) + refer_Force(i,1)*1000];
%     F_arraw_y = [position_hist(i,2), position_hist(i,2) + refer_Force(i,2)*1000];
%     quiver(position_hist(i,1),position_hist(i,2),refer_Force(i,1)*1000,refer_Force(i,2)*1000, 5)
%     hold off
% end
%% Simulink
